...
function processFunction(call, callback) {
    const inputRecords = call.request.records.map((record) => {
        return new record_1.Record(record);
    });
...
const serviceProto = grpc.loadPackageDefinition(packageDefinition).io.meroxa.funtime;
...
server.bindAsync(FUNCTION_ADDRESS, grpc.ServerCredentials.createInsecure(), () => {
    server.start();
