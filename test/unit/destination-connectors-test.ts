...
const testPipeline = { name: 'awake', uuid: 1 };

QUnit.module("Unit | Creating destinations", () => {
  QUnit.test("records: it creates a destination connector (postgres)", async (assert) => {
    assert.expect(8);
    ...
    const assertedMockClient = {
      connectors: {
        create: (connInput) => {
          ...
          assert.strictEqual(connInput.config['table.name.format'], 'rice-is-nice');
  ...
  QUnit.test("records: it creates a destination connector (mysql)", async (assert) => {
    assert.expect(8);
    ...
    const assertedMockClient = {
      connectors: {
        create: (connInput) => {
          ...
          assert.strictEqual(connInput.metadata['mx:connectorType'], 'destination');
          assert.strictEqual(connInput.pipeline_name, 'awake');
          assert.strictEqual(connInput.config['table.name.format'], 'potatoes-too');
          assert.notOk(connInput.config['collection']);
          assert.notOk(connInput.config['aws_s3_prefix']);
          assert.notOk(connInput.config['snowflake.topic2table.map']);
        }
    ...
    const assertedMockClient = {
      connectors: {
        create: (connInput) => {
          ...
          assert.strictEqual(connInput.metadata['mx:connectorType'], 'destination');
          assert.strictEqual(connInput.pipeline_name, 'awake');
          assert.strictEqual(connInput.config['table.name.format'], 'berriesallday');
          assert.notOk(connInput.config['collection']);
          assert.notOk(connInput.config['aws_s3_prefix']);
          assert.notOk(connInput.config['snowflake.topic2table.map']);
        }
  ...
  QUnit.test("records: it creates a destination connector (mongodb)", async (assert) => {
    assert.expect(8);
  ...
  QUnit.test("records: it creates a destination connector (s3)", async (assert) => {
    assert.expect(8);
  ...
  QUnit.test("records: it creates a destination connector (snowflakedb)", async (assert) => {
    assert.expect(8);
